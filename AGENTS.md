...
This file follows the [agents.md](https://agents.md) convention and is read automatically by Antigravity CLI, Cursor (newer versions), Codex, and any other agent that respects the standard. **It is kept in sync with [CLAUDE.md](CLAUDE.md) — both files describe the same behavior.** Pick the agent CLI you prefer; the workflow is identical.
...
- GOOD:
  ...
  > **2. Ask each time** — your CLI prompts before commands not pre-allowed.
  >
  > Which?
- BAD: "allow all or ask?"

> **Slash commands:** Claude Code registers `/GIF` natively via `.claude/commands/GIF.md`. If your agent does NOT register slash commands, type `/GIF` as a normal message — the agent will read this file, load the workflow, and start Phase 0. No functional difference.
...
Step 2: Check prerequisites → AUTO-INSTALL anything missing
...
**Permission mode is decided once, on the very first `/GIF` run, via Q0 of the first-run setup.** The choice is saved to `static-to-gif/stms-preferences.json` as `permission_mode`. Never re-asked.

**Two modes:**

1. **`allow_all`** (recommended, default selection) — the agent runs every command without prompting. Just execute. If a command fails, fix it and retry — don't ask if you're allowed to.

2. **`ask_each_time`** — your CLI prompts before each command. The agent must NOT try to bypass these prompts. If the user denies a command, do not retry the same command — ask the user how to proceed.

**Per-agent implementation:**

| Agent | How `allow_all` is enforced |
|---|---|
| **Claude Code** | Writes broad wildcards to `.claude/settings.local.json` (Bash, Read, Write, Edit, Glob, Grep, MCP). Already ships pre-configured. |
| **Antigravity CLI** | Uses Antigravity's own permission model — agent runs commands; user grants tool access through Antigravity's permission UI on first invocation. STMS does not override this. |
| **Cursor / other agents** | Use the agent's native permission model. STMS does not override. |

The Q0 question is still asked once and the user's intent (`allow_all` vs `ask_each_time`) is saved to preferences — but only Claude Code has a settings file STMS writes to directly. For other agents, the preference is informational; the actual prompts come from the host CLI.
...
**Note for non-Claude agents:** Skills are installed into `.agents/skills/` regardless of agent. Claude Code auto-discovers them; for other agents, the workflow file (`.agents/static-to-gif/workflow.md`) explicitly references the right skill at the right phase, so the agent can read them as needed.

---

## Slash Commands — EXECUTE IMMEDIATELY ON USE

When a user types any of these commands, **do NOT ask what they mean or explain how they work**. Load the referenced workflow file and start executing immediately. If your CLI doesn't register slash commands natively, the user will type `/GIF` as a message — treat it the same way.
...
- `.agents/` — workflow definitions, skills, STMS pipeline (canonical source of truth — agent-agnostic)
- `.claude/` — Claude Code-specific extras: slash command registration, native skills, permissions file
...
6. Phase 3 placement uses the 50% overlay method as PRIMARY AND ONLY technique. Optional one-shot CV accelerator for the background only. See `.agents/static-to-gif/workflow.md` Phase 3.
7. Never preview output to the user via OS image viewers, CLI image tools, or inline image embeds in the agent chat. Only `npm run dev` (HyperFrames Studio in the browser).
