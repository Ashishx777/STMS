...
Convert static designs + Figma-exported elements into animated GIFs.

**Command:** `/GIF`, `/GIF <path>`, or `/GIF --project <name>`

**Workflow:** Read `.agents/static-to-gif/workflow.md` before starting any STMS task.

```
assets/
  static-gif/
    <project-name>/             # Assets copied from source (never moved)
      static.png
      elements/
...
static-to-gif/
  ...
  output/                       # Final GIFs/MP4s
```

**Phases:**
0. **Smart File Discovery** — accept path or ask, scan source, auto-name, copy to `assets/static-gif/`
1. **Ingest** — inventory elements, check fonts
2. **Reconstruct** — pixel-perfect non-animated HTML composition from elements
3. **Verify** — overlay method against static reference, fix ALL misalignments
4. **Ask User** — which elements to animate, what animation type, timing
5. **Animate** — custom GSAP animations (no presets)
6. **Preview** — show user, iterate
7. **Render** — only on explicit user command
